...
@interface TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate,UIAlertViewDelegate> {
    UICollectionView *_collectionView;
    NSArray <TYimageModel *>*_photosTemp;
    NSArray *_assetsTemp;
    UIView *_naviBar;
    ...
    NSMutableArray  *_TZPHotoselectedModels;
...
- (void)configCustomNaviBar {
    ...
    _TZPHotoselectedModels = [NSMutableArray arrayWithArray:tzImagePickerVc.selectedModels];
    ...
    _deleteButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.tz_width - 54, 10, 42, 42)];
    [_deleteButton setImage:[UIImage imageNamed:@"delete_Photoo_Icon"] forState:UIControlStateNormal];
    [_deleteButton addTarget:self action:@selector(delete:) forControlEvents:UIControlEventTouchUpInside];
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.tz_width - 100) /2, 10, 100, 42)];
    _titleLabel.text = @"";
    _titleLabel.font = [UIFont systemFontOfSize:17];
    _titleLabel.textColor = [UIColor whiteColor];
    _titleLabel.textAlignment = NSTextAlignmentCenter;
    //如果是在预览状态下进来的则不显示 右上角的选择与反选按钮
...
//            删除该张图片
            ...
            for (TZAssetModel *model_item in _TZPHotoselectedModels) {
                if ([[[TZImageManager manager] getAssetIdentifier:model] isEqualToString:[[TZImageManager manager] getAssetIdentifier:model_item]]) {
                    
                    [_TZPHotoselectedModels removeObjectAtIndex:_currentIndex];
...
//                    }
            ...
            if (_TZPHotoselectedModels.count > 0) {
                [self refreshTitleLabel];
    ...
    switch (_tzPhotoType) {
        ...
        case 1:{
            if (self.doneButtonClickBlockWithPreviewType) {
                self.doneButtonClickBlockWithPreviewType(self.photos,_TZPHotoselectedModels,self.isSelectOriginalPhoto);
            }
        }
            break;
...
// 刷新Title Label 上显示的文字
-(void)refreshTitleLabel{
    _titleLabel.text = [NSString stringWithFormat:@"%ld/%lu",(long)_currentIndex + 1,(unsigned long)[_models count]];
