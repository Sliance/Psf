...
- (instancetype)initWithSelectedAssets:(NSMutableArray <TYImageAssetModel *>*)selectedAssets selectedPhotos:(NSMutableArray <TYimageModel *>*)selectedPhotos index:(NSInteger)index;
...
@property (nonatomic, copy) void (^didFinishPickingPhotosHandle)(NSArray<TYimageModel *> *photos,NSArray *assets,BOOL isSelectOriginalPhoto);
...
// 你可以通过一个asset获得原图，通过这个方法：[[TZImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray <TYImageAssetModel *>*)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray <TYImageAssetModel *>*)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
