...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = DSColorFromHex(0XF2F2F2 );
...
- (void) setImageWithImageModel:(TYimageModel *)imageModel{
    ...
    if (imageModel.imageUrl.length <7) { // 担心URL 为无效的 如 https://
        NSData * imageData = UIImageJPEGRepresentation(imageModel.image, 0.5);
        _imageView.image = [UIImage imageWithData:imageData];
    }else{
        [_imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",imageModel.imageUrl,@"?imageMogr/v2/thumbnail/50x50"]] placeholderImage:[UIImage imageNamed:@"dong"]];
    }
}
