...
- (void)viewDidLayoutSubviews
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 11.0) { // iOS 11
        ...
        _searchBar.py_width = SCREENWIDTH-73;
        _searchBar.py_height = 32;
...
- (void)setup
{
    ...
    for (UIView *subView in [[searchBar.subviews lastObject] subviews]) {
        if ([[subView class] isSubclassOfClass:[UITextField class]]) {
            ...
            textField.backgroundColor = DSColorFromHex(0xF0F0F0);
...
- (void)setupHotSearchRectangleTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rectangleTagLabel.backgroundColor = [UIColor redColor];
...
- (void)setupHotSearchRankTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        rankTextLabel.backgroundColor = [UIColor redColor];
...
//        NSArray *colorStrings = @[@"#f14230", @"#ff8000", @"#ffcc01", @"#ebebeb"];
        NSArray *colorStrings = @[@"#ffffff", @"#ffffff", @"#ffffff", @"#ffffff"];
...
- (UILabel *)labelWithTitle:(NSString *)title
{
    ...
    label.text = title;
    label.textColor = DSColorFromHex(0x464646);
    label.backgroundColor = [UIColor whiteColor];
    label.layer.cornerRadius = 4;
    label.clipsToBounds = YES;
    label.layer.borderWidth = 0.5;
    label.layer.borderColor = DSColorFromHex(0x464646).CGColor;
