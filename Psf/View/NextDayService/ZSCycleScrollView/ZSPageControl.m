...
- (void)initialization
{
    self.dotViewClass           = [ZSAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(ZSPageControl:didSelectPageAtIndex:)]) {
            [self.delegate ZSPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[ZSAnimatedDotView class]] && self.dotColor) {
            ((ZSAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
{
    if (self.dotViewClass) {
        ZSAbstractDotView *abstractDotView = (ZSAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [ZSAbstractDotView class]);
        }
